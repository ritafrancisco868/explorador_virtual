...
class ExploradorVirtual:
    ...
    def carregar_imagem(self, nome_pais):
        """Carrega a imagem do país atual ou uma imagem padrão."""
        try:
    ...
    def encontrar_pais_no_json(self, nome_desejado):
        ...
        variacoes = {
            'Inglaterra': ['Reino Unido', 'Grã-Bretanha', 'UK'],
            'Rússia': ['Russia', 'Federação Russa'],
            'Coreia do Sul': ['Coreia do Sul', 'Coréia do Sul', 'South Korea'],
            'Estados Unidos': ['EUA', 'USA', 'United States'],
        }
        
        if nome_desejado in variacoes:
            for variacao in variacoes[nome_desejado]:
                if variacao in self.paises:
                    return variacao
                for pais_json in self.paises.keys():
                    if self.normalizar_para_comparacao(pais_json) == self.normalizar_para_comparacao(variacao):
                        return pais_json
